...
int main() {
    ...
    cv::imwrite("result/task1_images/gray.png", gray);
    ...
    cv::imwrite("result/task1_images/hsv_h.png", hsv_channels[0]);
    cv::imwrite("result/task1_images/hsv_s.png", hsv_channels[1]);
    cv::imwrite("result/task1_images/hsv_v.png", hsv_channels[2]);
    ...
    cv::imwrite("result/task1_images/mean_filter.png", meanImg);

    // 高斯滤波 5x5, sigmaX=1.5
    cv::GaussianBlur(img, gaussianImg, cv::Size(5, 5), 1.5);
    cv::imwrite("result/task1_images/gaussian_filter.png", gaussianImg);

    // 中值滤波 核=5
    cv::medianBlur(img, medianImg, 5);
    cv::imwrite("result/task1_images/median_filter.png", medianImg);
    ...
    cv::imwrite("result/task1_images/red_mask.png", red_mask);
    ...
    cv::imwrite("result/task1_images/contours_boxes.png", contour_result);
    ...
    cv::imwrite("result/task1_images/drawing.png", draw_img);
    ...
    cv::imwrite("result/task1_images/rotated_35deg.png", rotated);

    // 5.3 裁剪左上角 1/4
    int w = img.cols / 2;
    int h = img.rows / 2;
    cv::Mat crop = img(cv::Rect(0, 0, w, h));
    cv::imwrite("result/task1_images/crop_top_left.png", crop);
